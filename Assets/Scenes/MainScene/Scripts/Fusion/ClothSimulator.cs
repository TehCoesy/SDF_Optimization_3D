        ...
        for (int i = 0; i < numParticles; i++) {
            for (int j = 0; j < collidableObjects.Length; j++) {
                ...
                if (collider.GetType() == typeof(SphereCollider)) {
                    ...
                    Vector3 forward = (projectedPositions[i] - positions[i]).normalized;
                    Vector3 o = positions[i] - forward * 500.0f;

                    RaycastHit hitInfo;

                    Ray rayForward = new Ray(positions[i], forward);
                    ...
                    if (Physics.Raycast(rayForward, out hitInfo, (projectedPositions[i] - positions[i]).magnitude)) {
                        hitForward = true;
                    ...
                    if (Physics.Raycast(rayBackward, out hitInfo, (o - positions[i]).magnitude)) {
                        hitBackward = true;
                    ...
                    if (hitForward && !hitBackward) {
                        collisionConstraints.Add(new MeshCollisionConstraint(i, pointForward, normalForward));
                    }
                    ...
                    if (!hitForward && hitBackward) {
                        ...
                        Vector3 o2 = projectedPositions[i] + forward * 500.0f;
                        Ray closest = new Ray(o2, (projectedPositions[i] - o2).normalized);
                        ...
                        distForward = (o2 - projectedPositions[i]).magnitude - distForward;
                        distBackward = (o - projectedPositions[i]).magnitude - distBackward;

                        if (distForward < distBackward) {
                            collisionConstraints.Add(new MeshCollisionConstraint(i, pointForward, normalForward));
                        } else {
                            collisionConstraints.Add(new MeshCollisionConstraint(i, pointBackward, normalBackward));
                        }
                    }
                }
