...
public class SphereCollisionConstraint : Constraint {
    ...
    public override void Satisfy(Vector3[] projectedPositions, float mass) {
        ...
        if (cp < 0) { // if constraint violated, project the constraint
            ...
            float distToTriangle = Vector3.Dot(collisionPosition - p, collisionNormal);
            // q is the closest point on the sphere's surface to p
            Vector3 q = sphereCenter + n * (sphereRadius + 0.001f);
            Vector3 n1 = collisionNormal + (projected - previous).normalized;
            //projectedPositions[vertexIndex] = q;
            projectedPositions[vertexIndex] += (distToTriangle + 0.001f) * collisionNormal;
        ...
        Vector3 point1 = previous, point2 = previous;
        float length1 = 0.0f, length2 = 0.0f;

        Ray rayForward = new Ray(previous, (projected - previous).normalized);

        Vector3 outsider = previous + (previous - projected).normalized * 500.0f;
        float length3 = (outsider - previous).magnitude;

        Ray rayBackward = new Ray(outsider, (previous - outsider).normalized);
        ...
        if (Physics.Raycast(rayBackward, out hit)) {
            hit2 = true;
            point2 = hit.point;
            length2 = hit.distance;
        }

        if (hit1 && hit2) {
            if (length3 - length2 > length1) {
                projectedPositions[vertexIndex] = projected - (projected - previous).normalized * 0.5f;
            } else {
                projectedPositions[vertexIndex] = previous + (previous - projected).normalized * 0.5f;
            }
        }
        */
    }
...
public class NewSphereCollisionConstraint : Constraint {
    ...
    public NewSphereCollisionConstraint(Vector3 x, Vector3 projectedPosition, int[] vertices, Vector3 center, float radius) {
        verticesIndex = vertices;
        sphereCenter = center;
        sphereRadius = radius;

        Vector3 direction = (projectedPosition - x).normalized;
        float L = (center - x).magnitude;

        float tc = Vector3.Dot(center - x, direction);
        float d = Mathf.Sqrt(L * L - tc * tc);
        float tc1 = Mathf.Sqrt(radius * radius - d * d);
        float t = tc - tc1;

        collisionPosition = x + direction * t;
        collisionNormal = (collisionPosition - center).normalized;
        projectedX = projectedPosition;
    }

    public override void Satisfy(Vector3[] projectedPositions, float mass) {
        float cp = Vector3.Dot(projectedX - collisionPosition, collisionNormal);

        if (cp < 0) { // if constraint violated, project the constraint
            Vector3 n = (projectedX - sphereCenter).normalized;
            // q is the closest point on the sphere's surface to p
            Vector3 q = sphereCenter + n * (sphereRadius + 0.001f);

            // Apply new projectedPositions to triangle's vertices
            projectedPositions[verticesIndex[0]] += (q - projectedX);
            projectedPositions[verticesIndex[1]] += (q - projectedX);
            projectedPositions[verticesIndex[2]] += (q - projectedX);
        }
    }
}
...
public class NewCubeCollisionConstraint : Constraint {
    public NewCubeCollisionConstraint() {
...
public class MeshCollisionConstraint : Constraint {
    ...
    public MeshCollisionConstraint(int index, Vector3 cPosition, Vector3 normal) {
        vertexIndex = index;
        collisionPosition = cPosition;
    ...
    public override void Satisfy(Vector3[] projectedPositions, float mass) {
        ...
        if (cp < 0) {
            float distToTriangle = Vector3.Dot(collisionPosition - p, collisionNormal);
            p += (distToTriangle + 0.1f) * collisionNormal;
            if ((p - projectedPositions[vertexIndex]).magnitude < 1.0f) {
                projectedPositions[vertexIndex] = p;
            }
...
public class NewMeshCollisionConstraint : Constraint {
    private int vertexIndex;
    private Vector3 collisionPosition;
    private Vector3 collisionNormal;
    ...
    public NewMeshCollisionConstraint(int index, Vector3 normal, float dist, int[] indices) {
        vertexIndex = index;
        collisionNormal = normal;
        collisionDist = dist;
    ...
    public override void Satisfy(Vector3[] projectedPositions, float mass) {
        ...
        if (collisionDist < threshold) {
            projectedPositions[triangleIndices[0]] += collisionDist * collisionNormal;
            projectedPositions[triangleIndices[1]] += collisionDist * collisionNormal;
            projectedPositions[triangleIndices[2]] += collisionDist * collisionNormal;
        } else {
            projectedPositions[triangleIndices[0]] += threshold * collisionNormal;
            projectedPositions[triangleIndices[1]] += threshold * collisionNormal;
            projectedPositions[triangleIndices[2]] += threshold * collisionNormal;
        }
    }
}
